...
const TransferScreen: React.FC = () => {
  ...
  const handleSendTransfer = async () => {
    if (productList.length > 0) {
      try {
        await axios.post('http://localhost:8000/api/transfers/send', { products: productList });
  ...
  const handleAcceptTransfer = async (index: number) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/transfers/accept', { products: transfer });
      if (missing.length > 0) {
